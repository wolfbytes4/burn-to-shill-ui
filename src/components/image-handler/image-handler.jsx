...
class ImgHandler extends Component {
  ...
  async fetchImage() {
    ...
    const nftMetaData = await queryTokenMetadata(
      p.token_id,
      null,
      p.selectedContract.burn_info.nft_contract.address,
      p.selectedContract.burn_info.nft_contract.code_hash
    );
    if (
      nftMetaData &&
  ...
  handleIndexedDb() {
    const p = this.props;
    const table = p.selectedContract.burn_info.nft_contract.name;
  ...
  async getImageByTokenId(db, token_id, table) {
    ...
    query.onsuccess = async (event) => {
      if (query.result && query.result.imageUrl) {
        db.close();
        if (p.selectedContract.burn_info.trait_restriction) {
          const attributes = JSON.parse(query.result.attributes);
          const hasAttribute = attributes.some(
            (f) =>
              f.trait_type === p.selectedContract.burn_info.trait_restriction
          );
